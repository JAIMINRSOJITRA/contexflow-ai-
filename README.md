...
⚠️ **IMPORTANT:** Before using this project, read [SECURITY_NOTICE.md](SECURITY_NOTICE.md) if you're setting up for the first time or have committed sensitive data.

ContextFlow AI is a Retrieval-Augmented Generation (RAG) system. You upload documents, it reads and indexes them, and then you can ask plain-English questions and get answers grounded in *your* documents, generated by either Gemini or Groq.
...
- **Project type:** AI-powered RAG system with FastAPI backend + Streamlit frontend
- **Core idea:** Retrieval-Augmented Generation over your own documents
- **Primary language:** Python
- **Backend:** FastAPI (REST APIs)
- **Frontend:** Streamlit (Web UI)
- **LLM providers:** Gemini and Groq (both working)
- **Current version:** 0.2.0
- **Current status:** Core pipeline built and functional; dual interface (API + UI)
...
├── README.md
├── requirements.txt
├── app.py                      # Streamlit frontend (primary UI)
...
⚠️ **Security Note:** Keep your API keys private. Never commit `.env` files to Git. See [SECURITY_NOTICE.md](SECURITY_NOTICE.md) for best practices.
...
- If keys are exposed, regenerate them immediately at the provider dashboards
...
### Option 1: Streamlit Frontend (Recommended for UI)
```cmd
streamlit run app.py
```
Opens in browser at `http://localhost:8501` with full UI.

### Option 2: FastAPI Backend Only (For API access)
```cmd
uvicorn app.main:app --reload
```
API at `http://127.0.0.1:8000` · Docs at `/docs` · Health check at `/health`

**Note:** The FastAPI backend provides REST APIs only. For a web interface, use the Streamlit frontend.
...
| 11 | Auth, rate limiting, deployment options | Not started |
